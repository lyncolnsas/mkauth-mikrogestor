...
class EVOLUTION2 {
	...
	public function gerarTXT($i, $t) : string {
		...
		if ($t == 1) {
			...
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_POSTFIELDS, json_encode(array('number' => '{$destino}', 'text' => '{$mensagem}', 'delay' => 1200)));" . PHP_EOL;
		...
		if ($t == 2) {
			...
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_CUSTOMREQUEST, 'POST');" . PHP_EOL;
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_HTTPHEADER, array('content-type: application/json', 'apikey: {$wapp_pass}'));" . PHP_EOL;
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_POSTFIELDS, json_encode(array('number' => '{$destino}', 'mediatype' => 'image', 'caption' => '{$legenda}', 'media' => \$anexo{$i}, 'delay' => 1200)));" . PHP_EOL;
			$cmd_evo .= "\$resposta{$i} = curl_exec(\$curl{$i});" . PHP_EOL;
			$cmd_evo .= "\$httpCode{$i} = curl_getinfo(\$curl{$i}, CURLINFO_HTTP_CODE);" . PHP_EOL;
			$cmd_evo .= "curl_close(\$curl{$i});" . PHP_EOL . PHP_EOL;
		...
		if ($t == 3) {
			...
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_CUSTOMREQUEST, 'POST');" . PHP_EOL;
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_HTTPHEADER, array('content-type: application/json', 'apikey: {$wapp_pass}'));" . PHP_EOL;
			$cmd_evo .= "curl_setopt(\$curl{$i}, CURLOPT_POSTFIELDS, json_encode(array('number' => '{$destino}', 'mediatype' => 'document', 'caption' => '{$legenda}', 'media' => \$anexo{$i}, 'fileName' => '{$arqnome}', 'delay' => 1200)));" . PHP_EOL;
			$cmd_evo .= "\$resposta{$i} = curl_exec(\$curl{$i});" . PHP_EOL;
			$cmd_evo .= "\$httpCode{$i} = curl_getinfo(\$curl{$i}, CURLINFO_HTTP_CODE);" . PHP_EOL;
			$cmd_evo .= "curl_close(\$curl{$i});" . PHP_EOL . PHP_EOL;
