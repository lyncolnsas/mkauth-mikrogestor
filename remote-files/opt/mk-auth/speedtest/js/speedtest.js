/** OPCOES **/
var openSpeedTestServerList = [{"ServerName":"Home", "Download":"downloading", "Upload":"upload", "ServerIcon":"DefaultIcon"}];
var pingSamples = 10;
			...
			if (typeof getCommand.host === "string" || typeof getCommand.h === "string") {
				...
				if (isValidHttpUrl(severAddress)) {
					openSpeedTestServerList = [{
						ServerName: "Home",
						Download: severAddress + "/downloading",
						Upload: severAddress + "/upload",
		...
		function OpenSpeedtest() {
			...
			var Engine = setInterval(function() {
				...
				if (Status === "Downloading") {
					...
					if (Startit == 0) {
						Startit = 1;
						Show.showStatus("Testando download speed..");
				...
				if (Status === "Uploading") {
					if (Startit == 1) {
						Startit = 2;
						Show.showStatus("Testando upload speed..");
						currentSpeed = 0;
						Get.reset();
						Show.reset();
				...
				if (Status === "Error") {
					...
					dummyElement.innerHTML = '<a xlink:href="./" style="cursor: pointer" target="_blank"></a>';
